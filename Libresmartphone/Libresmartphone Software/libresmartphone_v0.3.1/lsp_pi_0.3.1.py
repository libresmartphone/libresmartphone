...
##        self.setGeometry(300,300,210,200)
        ...
        global Tmissatges
        Tmissatges = QtGui.QTextEdit(TabSMS.widget(0))
        Tmissatges.resize(704,345)
        Tmissatges.move(0,5)
        Tmissatges.setReadOnly(True)
        Tmissatges.setFontPointSize(12)

        Lpn = QtGui.QLabel("Phone Number ", TabSMS.widget(1))
        Lpn.resize(90,30)
        Lpn.move(10,10)
        Lpn.setStyleSheet('font-size:10pt')

        global LEphn
        LEphn = QtGui.QLineEdit(TabSMS.widget(1))
        LEphn.resize (390,40)
        LEphn.move(110,5)
        LEphn.setStyleSheet('font-size:30pt')
        
        BSendMsg = QtGui.QPushButton("Send SMS",TabSMS.widget(1))
        BSendMsg.resize(190,40)
        BSendMsg.move(510,5)
        BSendMsg.setStyleSheet('font-size:30pt; background-color: orange')
        BSendMsg.clicked.connect(self.SendMsg)
        
        Ltm = QtGui.QLabel("Text Message ", TabSMS.widget(1))
        Ltm.resize(90,30)
        Ltm.move(10,50)
        Ltm.setStyleSheet('font-size:10pt')

        global TNewSms
        TNewSms = QtGui.QTextEdit(TabSMS.widget(1))
        ...
        Tecles = [
            ...
            ['q', 'w', 'e', 'r', 't', 'y', 'u', 'i', 'o', 'p'],
            ['a', 's', 'd', 'f', 'g', 'h', 'j', 'k', 'l', "'"],
            ['Up','z', 'x', 'c', 'v', 'b', 'n', 'm', '<--', ' Space'],
            ['@', '(', '%', '*', '/', '-', '+', ',', '.', '?'],
         ]

        teclat= QtGui.QWidget(TabSMS.widget(1))
        ...
        for idxLin,Lin in enumerate(Tecles):
            for idxTec, Tecla in enumerate(Lin):
                ...
                Tecla.clicked.connect(self.LLetra)
