...
#define MXS_AUART_RTSCTS    4  /* bit 4 */
    ...
    struct input_dev *clavier;
};

static unsigned char nkbd_keycode[64] = {
    	KEY_EQUAL, KEY_KPPLUS, KEY_KPASTERISK, KEY_1, KEY_4, KEY_SLASH, KEY_6, KEY_5,
    	KEY_F2, KEY_F1, KEY_F4, KEY_F3, KEY_F6, KEY_F5,	KEY_3, KEY_2,
	KEY_LEFTBRACE, KEY_ESC, KEY_EQUAL, KEY_RIGHTBRACE, KEY_DOT, KEY_0, KEY_BACKSPACE, KEY_DOWN,
    	KEY_KPLEFTPAREN, KEY_KPRIGHTPAREN, KEY_8, KEY_7, KEY_LEFT, KEY_9, KEY_RIGHT, KEY_UP,
	KEY_I, KEY_H, KEY_K, KEY_J, KEY_M, KEY_L, KEY_LEFTALT, KEY_N,
	KEY_B, KEY_A, KEY_D, KEY_C, KEY_F, KEY_E, KEY_ENTER, KEY_G,
	KEY_V, KEY_TAB, KEY_X, KEY_W, KEY_Z, KEY_Y, KEY_LEFTCTRL, KEY_LEFTSHIFT,
	KEY_P, KEY_O, KEY_R, KEY_Q, KEY_T, KEY_S, KEY_SPACE, KEY_U,
    };
...
static void mxs_auart_rx_char(struct mxs_auart_port *s)
{
    ...
    //printk("char clavier %c %d\n", c, c);

    int pressed;
    char key;

    printk("got value on UART : %d\n", (int)c);
	...
	printk("Reporting key %c, pressed : %d", key, pressed);
	input_report_key(s->clavier, key, pressed);
	//printk("Touche reconnue %d et valeur %d\n", key, c);
    //}
    //else 
    //{
       //printk("unknown char\n"); 
...
static void mxs_auart_settermios_clavier(struct uart_port *u);
...
mxs_auart_settermios_clavier(u);
}

static void mxs_auart_settermios_clavier(struct uart_port *u)
{
//printk("Termios %u %u\n", termios->c_cflag, termios->c_iflag);
printk("changing kbd bdrate\n");
    struct mxs_auart_port *s = to_auart_port(u);
    u32 bm, ctrl, ctrl2, div;
    unsigned int cflag, baud;
...
cflag = -2147480388;
    ...
    //uart_update_timeout(u, termios->c_cflag, baud);
    uart_update_timeout(u, cflag, baud);
...
static int mxs_auart_probe(struct platform_device *pdev)
{
    ...
    input_dev->name = "Awesome 2 keys Keyboard";
    //input_dev->phys = nkbd->phys;
    input_dev->id.bustype = BUS_RS232;
    input_dev->id.vendor = 10;
    ...
    int i;
    for (i = 0; i < 128; i++)
        set_bit(nkbd_keycode[i], input_dev->keybit);
    clear_bit(0, input_dev->keybit);
    int err = input_register_device(input_dev);
    if (err)
        printk("erreur init clavier %d", err);
    ...
    printk("calling init fnction\n");
    mxs_auart_settermios_clavier(&s->port);
printk("now opening uart\n");
mxs_auart_startup(&s->port);
